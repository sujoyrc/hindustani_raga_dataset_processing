...
class ProcessKeyPoints3D(object):
    ...
    def extract_keypoints(self,each_recording,fps):
        ...
        kp_file_full_name=os.path.join(self.start_directory,self.front_keypoints_directory,each_recording,kp_file_name)
        depth_file_name=each_recording+'_coords.npy'
        depth_file_full_name=os.path.join(self.start_directory,self.depth_keypoints_directory,depth_file_name)
...
def do_interpolate_and_lpf(keypoint_df):
    interpolated_df=keypoint_df.copy(deep=True)
    for each_keypoint_coord in self.keypoint_coord_columns:
...
def main():
    ...
    start_directory='./'
    front_keypoints_directory='final_output_folder'
    depth_keypoints_directory='output_np'
    output_dir='./process_3d_data_v3'

    for each_recording in sorted(os.listdir(front_keypoints_directory_full)):
        if type_of_processing=='3D':
            ...
            processKeyPoints2D=ProcessKeyPoints2D()
            processKeyPoints2D.run(each_recording)        

        interpolated_df=do_interpolate_and_lpf(this_keypoint_df)
